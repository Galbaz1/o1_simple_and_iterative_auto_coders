...
# Dictionary to represent a fake database for users (replace with real DB queries)
users_db = {}
...
async def upload_file(
    ...
    background_tasks.add_task(
        process_file, contents, file.content_type, file.filename, summary_level, language, current_user, request
    )
...
async def process_file(contents, content_type, filename, summary_level, language, user, request):
    try:
        ...
        if not text:
            raise HTTPException(
                status_code=400,
                detail="The uploaded file doesn't contain extractable text."
            )
        ...
        # Notify user or update session
        request.session['summary'] = final_summary

    except Exception as e:
        logger.error(f"Error processing file: {e}")
        request.session['summary_error'] = str(e)

@app.get("/summary/", response_class=HTMLResponse)
async def view_summary(request: Request):
    summary = request.session.get('summary', None)
    error = request.session.get('summary_error', None)
    user = await manager.get_current_user(request)

    if error:
        return templates.TemplateResponse(
            "error.html",
            {"request": request, "detail": error},
            status_code=500
        )
    ...
    # Clear the session summary to prevent reloading
